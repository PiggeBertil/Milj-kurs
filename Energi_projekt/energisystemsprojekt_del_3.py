...
Co2_before = 138772549.7714789 # ton
...
def prodDemand_rule(model, nodes, hours):
    s = 0
    for g in model.gens:
        s += model.prod[nodes, g, hours]


    for t in model.transmisionlines:
        if t[0:2] == nodes:
            s -= model.transmisions[t, hours]
        elif t[2:4] == nodes:
            s += model.transmisions[t, hours]*0.98


    return s - 0.1*model.prod[nodes, "Battery", hours] >= model.demand[nodes, hours]
...
def battery_flow_rule(model, hours, nodes):
    if hours != model.hours.last():  # Only apply if it's not the last hour
        produced = sum([model.prod[nodes, g, hours] for g in model.gens]) - model.prod[nodes, "Battery", hours]*1.1
        for t in model.transmisionlines:
            if t[2:4] == nodes:
                produced += model.transmisions[t, hours]*0.98
            elif t[0:2] == nodes:
                produced -= model.transmisions[t, hours]
...
def co2Max_rule(model):
    ...
    for node in model.nodes:
        for hour in model.hours:
            CO2 += model.prod[node, "Gas", hour]*0.202/0.4
    return CO2 <= Co2_cap
