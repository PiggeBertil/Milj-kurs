...
if __name__ == '__main__':
    ...
    plt.savefig("1_Production_DE_147-651.png", dpi = 300)
    plt.show()
    ...
    plt.savefig("1_Capacity_per_tech.png", dpi = 300)
    ...
    plt.savefig("1_Production_per_tech.png", dpi = 300)
    plt.show()

    for node in countries:
        plt.plot(input_data['Wind_' + node].values[147:651] * tech_data["Wind"]["cap"][node], label=[node])
    plt.legend()

    plt.title("Wind during three weeks in january")

    plt.savefig("Wind.png", dpi = 300)
    plt.show()

    for node in countries:
        plt.plot(input_data['PV_' + node].values[147:651] * tech_data["PV"]["cap"][node], label=[node])
    plt.legend()

    plt.title("Solarpower during three weeks in january")
    plt.savefig("Sun.png", dpi = 300)
